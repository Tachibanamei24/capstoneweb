...
function renderTeacherSchedulePage() {
    ...
    <div class="panel">
      ...
      <div class="schedule-form-grid">
        ...
        <div class="form-group">
          ...
          <input type="text" id="slot-room" value="${teacherSession.assignedRooms[0] || ""}" />
        </div>
  ...
  function resetForm() {
    ...
    document.getElementById("slot-room").value = teacherSession.assignedRooms[0] || "";
...
function initTeacherAutoRefresh() {
  ...
  teacherAutoRefreshInterval = setInterval(() => {
    const route = getTeacherRoute();
    if (!shouldAutoRefreshTeacherRoute(route)) return;
    const pageEl = document.getElementById("page-content");
    if (pageEl) pageEl.classList.add("refreshing");
    // Pull latest events and then re-render current teacher route to reflect new data.
