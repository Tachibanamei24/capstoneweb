...
async function renderDashboard() {
  ...
  try {
    ...
    document.getElementById("page-content").innerHTML = `
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="value">${stats.incidentsToday}</div>
        </div>
        <div class="stat-card">
          <div class="label">Red-level alerts this week</div>
          <div class="value red">${stats.redAlertsWeek}</div>
        </div>
        <div class="stat-card">
          <div class="label">Most noisy classroom</div>
          <div class="value">${stats.mostNoisyRoom}</div>
          <div style="font-size:0.8rem;color:var(--muted)">${stats.mostNoisyCount} incidents</div>
        </div>
        <div class="stat-card">
          <div class="label">Peak noise time</div>
          <div class="value green">${stats.peakTime}</div>
        </div>
      </div>
    ...
    chartInstances.push(
      new Chart(document.getElementById("chart-datetime"), {
        type: "line",
        data: {
...
async function renderReports() {
  ...
  try {
    ...
    document.getElementById("export-csv").addEventListener("click", () => exportCsv(logs));
    document.getElementById("export-pdf").addEventListener("click", () => {
      alert("PDF export: connect to your reporting endpoint when ready.");
    });
  } catch (e) {
    showError(e.message);
  }
...
function exportCsv(logs) {
  const headers = [
    "Date", "Time", "Room", "Device", "dB", "Status", "Warning Level",
    "Buzzer", "Audio", "Duration", "Subject", "Teacher", "Audio URL",
  ];
  const rows = logs.map((l) =>
    [
      l.date, l.time, l.room, l.deviceId, l.db, l.status, l.warningLevel || "",
      l.buzzer, l.audioRecorded, l.durationSec, l.subject, l.teacher, l.audioUrl || "",
    ].join(",")
  );
  ...
  a.download = `noise_events-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
}
...
function initAutoRefresh() {
  ...
  autoRefreshInterval = setInterval(() => {
    ...
    // Pull latest noise events from the server/cache and then re-run the renderer.
